...
function EditSongScreen({ editingTrack, tracks, currentTrack, t, getTrackGenre, setTracks, setCurrentTrack, saveTracks, goBack }: EditSongScreenProps) {
...
const THEMES: Record<Theme, ThemeConfig> = {
  ...
  black: {
    ...
    accent: "#FF6B00",
    artShadow: "0 24px 80px -12px rgba(255,107,0,0.4)",
    toggleOn: "#FF6B00",
    toggleOff: "#3A3A3C",
    thumbBg: "#FFFFFF",
    name: "Space Black",
...
export default function App() {
  ...
  }, [currentTrack.id, skipNext, repeat, audioRef]);
      ...
      >
        ...
        <div className="flex-1 flex items-center justify-center px-8 relative" style={{ minHeight: 0 }}>
          <VUMeter
            ...
            active={screen === "nowplaying" && isPlaying}
      ...
      >
        ...
        {!isLandscape && (
          <div className="flex items-center justify-between px-4 pt-1 pb-2 flex-shrink-0">
            ...
            >
              <ChevronLeft size={22} strokeWidth={2.5} />
              <span style={{ fontFamily: "Inter, sans-serif", fontSize: 15, fontWeight: 500, color: "#FF6B00" }}>
                Back
              </span>
            </button>
            <span
                ...
                {tracks.slice(0, Math.min(tracks.length, 80)).map((_, i) => (
                  <div
                    ...
                    style={{
                      ...
                      height: 5,
                      backgroundColor: i === cflowIndex ? "#FF6B00" : "rgba(255,255,255,0.22)",
                    }}
                  />
                ))}
              </div>
        ...
        {isLandscape && (
          <div className="flex flex-row w-full px-12 items-center" style={{ height: "40%", backgroundColor: "#000000" }}>
            ...
            <div className="flex-1 flex flex-col justify-center" style={{ alignItems: "flex-start" }}>
              ...
              <p style={{ fontFamily: "Inter, sans-serif", fontSize: 16, color: "#FF6B00", fontWeight: 500 }}>
                {cfTrack.artist}
              </p>
            </div>
  ...
  // ── Screen Router ───────────────────────────────────────────
  const ScreenHeader = ({ title, onBack = goBack }: { title: string; onBack?: () => void }) => (
    <div
      className="flex items-center px-4 pt-4 pb-3 flex-shrink-0 select-none"
      style={{ borderBottom: `1px solid ${t.divider}` }}
    >
  ...
  const SongTile = ({ track, onPlay, onOptions }: { track: Track; onPlay: () => void; onOptions: () => void }) => {
  ...
  const renderScreen = () => {
    switch (screen) {
      case "home":
        return HomeScreen();
      case "music":
        return ListScreen({ title: "Music", items: musicMenuItems });
      case "settings":
      ...
      case "artistSongs": {
        ...
        return (
          <div className="flex flex-col flex-1 overflow-hidden" style={{ backgroundColor: t.bg }}>
            <ScreenHeader title={selectedArtist} />
            <VirtualizedTrackList
              tracks={artistTracks}
              renderItem={(track) => (
                <SongTile
      ...
      case "albumSongs": {
        ...
        return (
          <div className="flex flex-col flex-1 overflow-hidden" style={{ backgroundColor: t.bg }}>
            <ScreenHeader title={selectedAlbum} />
            <VirtualizedTrackList
              tracks={albumTracks}
              renderItem={(track) => (
                <SongTile
      ...
      case "genreSongs": {
        ...
        return (
          <div className="flex flex-col flex-1 overflow-hidden" style={{ backgroundColor: t.bg }}>
            <ScreenHeader title={selectedGenre} />
            <VirtualizedTrackList
              tracks={genreTracks}
              renderItem={(track) => (
                <SongTile
      ...
      case "playlistSongs": {
        ...
        return (
          <div className="flex flex-col flex-1 overflow-hidden" style={{ backgroundColor: t.bg }}>
            <ScreenHeader title={selectedPlaylistName} />
